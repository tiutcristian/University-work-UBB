...
int sum = 0;
int state = EMPTY;		// buffer states: 0 = empty, 1 = full
int buf[MAX_SIZE];
...
void* produce(void* a)
{
	...
	while (state == FULL)				// if buffer state is FULL
...
{
	pthread_mutex_lock(&mtx);
	while (state == EMPTY)				// if buffer state is EMPTY
	...
	state = EMPTY;						// buffer is now EMPTY
	pthread_cond_broadcast(&cv);
	pthread_mutex_unlock(&mtx);
