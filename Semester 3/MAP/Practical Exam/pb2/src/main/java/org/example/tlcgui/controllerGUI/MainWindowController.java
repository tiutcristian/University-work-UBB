...
public class MainWindowController implements Initializable {
    ...
    private void initializeBarrierTableView() {
        // TableView with three columns: an index, a value and a list of values
        barrierTableView.getColumns().get(0).setText("Index");
        barrierTableView.getColumns().get(1).setText("Value");
        barrierTableView.getColumns().get(2).setText("List");

        barrierTableView.getColumns().get(0).setCellValueFactory(data ->
                new ObservableValueBase() {
                    @Override
                    public Integer getValue() {
                        return data.getValue().getKey();
                    }
                }
        );
        barrierTableView.getColumns().get(1).setCellValueFactory(data ->
                new ObservableValueBase() {
                    @Override
                    public Integer getValue() {
                        return data.getValue().getValue().getKey();
                    }
                }
        );
        barrierTableView.getColumns().get(2).setCellValueFactory(data ->
                new ObservableValueBase() {
                    @Override
                    public List<Integer> getValue() {
                        return data.getValue().getValue().getValue();
                    }
                }
        );

        barrierTableView.getColumns().get(0).prefWidthProperty().bind(barrierTableView.widthProperty().multiply(0.33));
        barrierTableView.getColumns().get(1).prefWidthProperty().bind(barrierTableView.widthProperty().multiply(0.33));
        barrierTableView.getColumns().get(2).prefWidthProperty().bind(barrierTableView.widthProperty().multiply(0.33));

        barrierTableView.setItems(prgStateService.getBarrierTableContent());
    }
